...
as
BEGIN
DECLARE @d varchar(100), @LD int, @m int, @n int, @i int, @j int,
...
WHILE @i<=@m
	BEGIN
	SET @d=STUFF(@d,@i*@m+1,1,CHAR(@i))--d(0, j) = j
	SET @i=@i+1
	END
--goto done
--Step 3
...
--Step 4
	...
	WHILE @j<=@m
		...
		ELSE
			SET @cost=1
--Step 6
		SET @d=STUFF(@d,@j*(@m)+@i+1,1,CHAR(dbo.MIN3(
ASCII(substring(@d,@j*(@m)+@i-1+1,1))+1,
ASCII(substring(@d,(@j-1)*(@m)+@i+1,1))+1,
ASCII(substring(@d,(@j-1)*(@m)+@i-1+1,1))+@cost)
))
		SET @j=@j+1
		END
	SET @i=@i+1
...
--WHILE @j<=@m
--	BEGIN
--		set @z=@z+CONVERT(char(1),ASCII(substring(@d,@i*(@m+1)+@j+1,1)))
--		SET @j=@j+1 
--		END
--	SET @i=@i+1
--	END
